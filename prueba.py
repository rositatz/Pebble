...
def escenarios_para_tipo(tipo_relacion):
    ...
    "Nuevas amistades" / "Sin definir"). Si no matchea nada (tipo vacío,
    "Sin definir", o un valor que no está en ningún escenario) corre todos."""
...
def actualizar_memoria(memoria, analisis):
    """Si es la primera interacción entre este par de gemelos, memoria todavía
    no existe (None) -- se crea acá. Si ya existía, se le hace append del
    resumen de esta nueva interacción arriba de las anteriores."""
...
def simular_cita(perfil1, perfil2, turnos=3, escenario=0, memoria1=None, memoria2=None):
    ...
    con el otro (ver actualizar_memoria) -- se usan para que, al correr varios
    escenarios seguidos (ver simular_relacion_completa), el gemelo no arranque
    de cero cada vez sino que la charla se sienta continuada."""
...
def simular_relacion_completa(uid1, perfil1, uid2, perfil2, tipo_relacion=None, turnos=2, umbral=0.75, guardar=guardar_simulacion_local):
    """Corre TODOS los escenarios que correspondan al tipo de relación que se
    busca -- no solo uno. Antes esto estaba fijo en escenario=0 (siempre
    "Arte y tecnología"); ahora usa escenarios_para_tipo() para elegir el
    conjunto entero.

    tipo_relacion: "Algo serio" / "Algo casual" / "Nuevas amistades" / None.
    Si no se pasa, se toma de perfil1["busco"] o perfil2["busco"] (ese campo
    hoy vive en el doc de cuenta de perfil.html, no en el perfil del gemelo
    generado por gemelo_perfil.py -- hay que fusionarlos antes de llamar acá
    una vez que esto corra en la Cloud Function).

    La memoria de cada gemelo se acumula escenario a escenario: la segunda
    simulación ya "recuerda" cómo fue la primera, y así siguiendo."""
