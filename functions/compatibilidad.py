...
def _temas_obligatorios(perfil1, perfil2, nombre1=None, nombre2=None, top_n=3):
    ...
    if perfil1.get("conflictos") or perfil2.get("conflictos"):
        disponibles.append(
            "Un desacuerdo o conflicto REAL entre ustedes dos -- un choque "
            "que pasa AHORA, en esta misma charla, sobre algo CONCRETO y "
            "sustancial: una diferencia real de opinión, valores, gustos o "
            "forma de ver algo (usen los VALORES PERSONALES, creencias, "
            "postura sobre hijos/futuro, o las DIFERENCIAS REALES DE "
            "PERSONALIDAD de más arriba -- lo que aplique -- como el motivo "
            "concreto de fondo, nunca un motivo inventado). El desacuerdo "
            "NUNCA puede ser sobre cómo se comunican, cómo manejan los "
            "conflictos, ni sobre necesitar espacio/distancia/que le avisen "
            "antes de algo -- ESO no es un tema de discusión, es solo el "
            "COMPORTAMIENTO con el que cada uno reacciona (ver más abajo). "
            "Prohibido que discutan o negocien EN VOZ ALTA sobre su forma de "
            "manejar conflictos ('necesito espacio', 'avisame antes de "
            "alejarte', 'prefiero que...') -- eso convierte el proceso en el "
            "contenido de la pelea, cuando tiene que ser solo la FORMA en "
            "que la viven, nunca el tema del que hablan.\n"
            "    MUY IMPORTANTE: 'CÓMO MANEJA LOS CONFLICTOS' (en el perfil "
            "de cada uno) es una guía de COMPORTAMIENTO, no de diálogo -- "
            "define cómo actuás mientras estás en desacuerdo sobre el tema "
            "real de arriba, nunca de qué hablan. Si tus datos dicen que "
            "necesitás distancia o te cuesta abrirte, ESO se actúa (mensajes "
            "más cortos/secos, tardar en responder, cambiar de tema para no "
            "seguir, o directamente decir que preferís cortar la charla por "
            "ahora) -- nunca lo anunciás ni lo pedís como si fuera parte de "
            "la negociación ('necesito que me des espacio'). Si tus datos "
            "dicen que confrontás directo, decís las cosas de frente SOBRE "
            "EL TEMA REAL, no sobre cómo el otro se comunica. No mezcles: la "
            ...
            "genérica de 'alguien peleando', y siempre sobre un tema real, "
            "no sobre el proceso de pelear en sí.\n"
            "    TAMPOCO narres tu propia reacción en términos clínicos/de "
            "terapia ('me lo tomé a pecho', 'me pongo a la defensiva', "
            "'estoy poniendo un límite', 'necesito procesar esto') -- eso es "
            "tan poco natural como explicar la consigna en voz alta. Mostrá "
            "la reacción en CÓMO hablás (más cortante, más callado/a, un "
            "silencio, un cambio de tono), nunca nombrando el fenómeno "
            "psicológico que te está pasando.\n"
            "    El desenlace de este desacuerdo NO tiene que resolverse "
            "bien por default -- depende de la compatibilidad real de "
            "arriba: cuanto más alta, más probable que de verdad lo hablen y "
            "lleguen a algo (un acuerdo genuino, entender al otro); cuanto "
            "más baja, más probable que quede sin resolver de verdad -- se "
            "diluye sin que nadie ceda, uno lo deja pasar sin estar "
            "realmente de acuerdo, o sigue un resto de tensión de fondo el "
            "resto de la charla. Forzar una reconciliación prolija sea cual "
            "sea la compatibilidad es tan poco realista como que nunca haya "
            "conflicto."
        )
